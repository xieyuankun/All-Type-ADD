...
def init():
    ...
    # 将 JSON 文件中的参数添加到 ArgumentParser 对象中
...
def extract_feats_and_labels(model, testDataLoader, args, task_type):
    ...
    all_feats = np.vstack(all_feats)  # 将特征数组堆叠成一个大的矩阵
    all_labels = np.array(all_labels)
    all_task_types = np.array(all_task_types)  # 转换为 NumPy 数组
...
def plot_tsne(all_feats, all_labels, all_task_types):
    ...
    label_markers = {0: 'x', 1: 'o'}  # 真实用 'x'，虚假用 'o'
    ...
    for task_type in np.unique(all_task_types):
        ...
        for label in [0, 1]:
            ...
            mask = task_mask & label_mask  # 合并任务类型和标签的掩码
